...
from vespernet.utils import apply_postprocess, build_model_from_config, load_config, save_csv, save_image, save_json
...
def main() -> None:
    parser = argparse.ArgumentParser(description="P3 clean inference.")
    parser.add_argument("--config", type=str, default="configs/inference_p3_clean.yaml")
    ...
    with torch.inference_mode():
        for batch in tqdm(loader, desc="P3 inference", ncols=80):
            low = batch["low"].to(device, non_blocking=True)
            name = batch["name"][0]
            prediction = model(low)["I_hat"][0]
